		...
		Photo.find({albumId: albumId}, function (err, data) {
			if (err) {
				...
				FlockAlbum.find({albumId: albumId}, function (err1, data1) {
					...
					Flock.update({openGId: {$in: ids}}, {$inc: {photoNum: deleteNum}}, function (e, v) {
						//console.log(v);
						for (var i = 0; i < data1.length; i++) {
					...
					FlockAlbum.remove({albumId: albumId}, function (e,v) {
					});
				});

				AlbumUser.remove({albumId: albumId}, function (e,v) {
				});
				//删除相册表中的数据
				Album.remove({_id: mongoose.Types.ObjectId(albumId)}, function (e,v) {
				});
				//删除Photo表中的数据
				Photo.remove({albumId: albumId}, function (e,v) {
				});
				//删除万象优图的数据
				for (var i = 0; i < data.length; i++) {
					functions.deletePicFromWxyt(data[i].fileId, function (e,v) {
					});
				}
			}
		});
...
router.get("/viewalbum/:albumId", function (req, res, next) {
	...
	Album.findOne({_id: mongoose.Types.ObjectId(albumId)}, function (err, data) {
		if (err) {
			...
			PatchPhoto.find({albumId: albumId,isShow:1}).sort({createTime: -1}).skip(start).limit(10).exec(function (err1, docs1) {
				if (err1) {
					res.status(200).json(errors.error3);
				} else {
					//判断是否是自己的
